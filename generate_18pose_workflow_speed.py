...
def generate_workflow():
    ...
    print("🚀 Generating 18-Pose SPEED Workflow...")
    print("=" * 60)
    print("⚡ Optimized for 20-minute timeout limit")
    print("   Settings: 12 steps | CFG 3.0 | LCM sampler")
    print("=" * 60)
    ...
    workflow["nodes"].append({
        ...
        "order": 0,
        "mode": 0,
        "title": "🚀 18-Pose Character Sheet Generator - SPEED VERSION",
        "properties": {},
        "widgets_values": [
            "⚡ FAST 18-POSE GENERATION - Optimized for 20min timeout\\n" +
            "🔧 Speed Settings: Denoise 0.70 | CFG 3.0 | Steps 12 | LCM Sampler\\n" +
            "📦 Output: 18 individual poses (Batch A: 10 body shots | Batch B: 8 close-ups)\\n" +
            "⏱️ Estimated time: 6-10 minutes (vs 20+ for quality version)"
        ],
        "color": "#1a5490",
        "bgcolor": "#2d6ca8"
    })
    ...
    for pose_idx, (pose_name, pose_desc) in enumerate(POSES):
        ...
        workflow["nodes"].append({
            ...
            "widgets_values": [123456 + pose_idx, "fixed", 12, 3.0, "lcm", "normal", 0.70],
            "title": f"⚡ {pose_name[:15]}"
        })
        ...
        workflow["nodes"].append({
            ...
            "widgets_values": [f"18Pose_SPEED_{pose_name}"],
            "title": f"💾 {pose_name}"
        })
        node_id += 1
...
if __name__ == "__main__":
    ...
    output_file = "18-Pose-SPEED-Workflow.json"
